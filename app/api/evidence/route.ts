...
export async function POST(req:Request){
 ...
 try{
  ...
  if(file.size>MAX_FILE_SIZE||!ALLOWED.has(file.type))return NextResponse.json({ok:false,error:'Chỉ nhận JPG, PNG, WEBP, PDF tối đa 8 MB'},{status:400});
  const path=`${module}/${legacyId}/${crypto.randomUUID()}-${safeName(file.name)}`;
  const {error:uploadError}=await db.storage.from('documents').upload(path,file,{contentType:file.type,cacheControl:'3600',upsert:false});
  ...
  if(error){await db.storage.from('documents').remove([path]);throw error;}
...
export async function GET(req:Request){
 ...
 try{
  ...
  const files=await Promise.all((data||[]).map(async f=>{const {data:signed}=await db.storage.from('documents').createSignedUrl(f.storage_path,600);return {...f,storage_path:undefined,url:signed?.signedUrl||''}}));
